  ...
  def _train_step(self, train_states, target_qf_params, rng, batch):
    ...
    def loss_fn(train_params, rng):
      ...
      if self.config.use_cql:
        ...
        if self.config.importance_sample:
          random_density = np.log(0.5**self.action_dim)
          cat_q1 = jnp.concatenate(
            [
              q1_rand - random_density, q1_next_actions - next_log_pis,
              q1_current_actions - current_log_pis
            ],
            axis=1
          )
          cat_q2 = jnp.concatenate(
            [
              q2_rand - random_density, q2_next_actions - next_log_pis,
          ...
          cat_q1 = jnp.concatenate(
            [
              q1_rand,
              jnp.expand_dims(q1_pred, 1), q1_next_actions, q1_current_actions
            ],
            axis=1
          )
          cat_q2 = jnp.concatenate(
            [
              q2_rand,
              jnp.expand_dims(q2_pred, 1), q2_next_actions, q2_current_actions
            ],
            axis=1
          )
        ...
        if self.config.lagrange:
          ...
          min_qf1_loss = alpha_prime * self.config.min_q_weight * (
            qf1_diff - self.config.target_action_gap
          )
          min_qf2_loss = alpha_prime * self.config.min_q_weight * (
