...
class NoiseScheduleVP:
  ...
  def __init__(
    ...
    if schedule == 'discrete':
      ...
      self.cosine_t_max = math.atan(
        self.cosine_beta_max * (1. + self.cosine_s) / math.pi
  ...
  def marginal_log_mean_coeff(self, t):
    ...
    if self.schedule == 'discrete':
      ...
      return interpolate_fn(
        t.reshape((-1, 1)), self.t_array, self.log_alpha_array
      ).reshape(t_shape)
    elif self.schedule == 'linear':
      return -0.25 * t**2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
    elif self.schedule == 'cosine':
      log_alpha_fn = lambda s: jnp.log(
        jnp.cos((s + self.cosine_s) / (1. + self.cosine_s) * math.pi / 2.)
      )
  ...
  def inverse_lambda(self, lamb):
    ...
    if self.schedule == 'linear':
      tmp = 2. * (self.beta_1 -
                  self.beta_0) * jnp.logaddexp(-2. * lamb, jnp.zeros((1,)))
      Delta = self.beta_0**2 + tmp
      return tmp / (jnp.sqrt(Delta) + self.beta_0) / (
      ...
      t = interpolate_fn(
        log_alpha.reshape((-1, 1)), jnp.flip(self.log_alpha_array, [1]),
        jnp.flip(self.t_array, [1])
      )
      return t.reshape(shape)
    else:
      log_alpha = -0.5 * jnp.logaddexp(-2. * lamb, jnp.zeros((1,)))
      t_fn = lambda log_alpha_t: jnp.arccos(
        jnp.exp(log_alpha_t + self.cosine_log_alpha_0)
      ) * 2. * (1. + self.cosine_s) / math.pi - self.cosine_s
      t = t_fn(log_alpha)
      return t
...
def model_wrapper(
  ...
  def noise_pred_fn(x, t_continuous, cond=None):
    ...
    if model_type == "noise":
      ...
      dims = x.ndim
      return (x - expand_dims(alpha_t, dims) *
              output) / expand_dims(sigma_t, dims)
    elif model_type == "v":
      alpha_t, sigma_t = noise_schedule.marginal_alpha(
        t_continuous
      ), noise_schedule.marginal_std(t_continuous)
      dims = x.ndim
      return expand_dims(alpha_t,
                         dims) * output + expand_dims(sigma_t, dims) * x
  ...
  def cond_grad_fn(x, t_input):
    """
        ...
        """
    log_prob = lambda x_in: classifier_fn(
...
class DPM_Solver:
  ...
  def get_time_steps(self, skip_type, t_T, t_0, N):
    ...
    if skip_type == 'logSNR':
      ...
      t_order = 2
      t = jnp.linspace(t_T**(1. / t_order), t_0**(1. / t_order),
                       N + 1).pow(t_order)
      return t
    else:
      raise ValueError(
  ...
  def get_orders_and_timesteps_for_singlestep_solver(
    self, steps, order, skip_type, t_T, t_0
  ):
    """
    ...
    if skip_type == 'logSNR':
      ...
      timesteps_outer = self.get_time_steps(skip_type, t_T, t_0,
                                            steps)[jnp.cumsum(
                                              jnp.array([
                                                0,
                                              ] + orders)
  ...
  def singlestep_dpm_solver_second_update(
    ...
    log_alpha_s, log_alpha_s1, log_alpha_t = ns.marginal_log_mean_coeff(
      s
    ), ns.marginal_log_mean_coeff(s1), ns.marginal_log_mean_coeff(t)
    ...
    if self.predict_x0:
      ...
      if solver_type == 'dpm_solver':
        ...
        x_t = (
          ...
          expand_dims(alpha_t * ((jnp.exp(-h) - 1.) / h + 1.), dims) *
          (model_s1 - model_s)
        )
    else:
      phi_11 = jnp.expm1(r1 * h)
      ...
      if solver_type == 'dpm_solver':
        ...
        x_t = (
          ...
          expand_dims(sigma_t * ((jnp.exp(h) - 1.) / h - 1.), dims) *
          (model_s1 - model_s)
        )
    if return_intermediate:
  ...
  def singlestep_dpm_solver_third_update(
    ...
    log_alpha_s, log_alpha_s1, log_alpha_s2, log_alpha_t = ns.marginal_log_mean_coeff(
      s
    ), ns.marginal_log_mean_coeff(s1), ns.marginal_log_mean_coeff(
      s2
    ), ns.marginal_log_mean_coeff(t)
    sigma_s, sigma_s1, sigma_s2, sigma_t = ns.marginal_std(s), ns.marginal_std(
      s1
    ), ns.marginal_std(s2), ns.marginal_std(t)
    alpha_s1, alpha_s2, alpha_t = jnp.exp(log_alpha_s1), jnp.exp(
      log_alpha_s2
    ), jnp.exp(log_alpha_t)
  ...
  def multistep_dpm_solver_third_update(
    ...
    lambda_prev_2, lambda_prev_1, lambda_prev_0, lambda_t = ns.marginal_lambda(
      t_prev_2
    ), ns.marginal_lambda(t_prev_1), ns.marginal_lambda(
      t_prev_0
  ...
  def dpm_solver_adaptive(
    ...
    def update_fn(val):
      ...
      delta = jnp.maximum(
        jnp.ones_like(x) * atol,
        rtol * jnp.maximum(jnp.abs(x_prev), jnp.abs(x_lower))
      )
      norm_fn = lambda v: jnp.sqrt(
        jnp.
        mean(jnp.square(v.reshape((v.shape[0], -1))), axis=-1, keepdims=True)
      )
      ...
      x, x_prev, s, lambda_s = jax.lax.cond(
        E <= 1., next_step, remain_step, ()
      )
      h = jnp.minimum(
        theta * h * jnp.power(E, -1. / order), lambda_0 - lambda_s
      )
      nfe = nfe + order
    ...
    x, _, _, _, _, nfe = jax.lax.while_loop(
      condition_continue, update_fn, (x, x_prev, s, lambda_s, h, 0)
    )
    ...
    if method == 'adaptive':
      ...
      def multistep_loop_fn(step, val):
        ...
        models_prev = models_prev.at[-1].set(
          jax.lax.cond(
            step < steps, lambda _: self.model_fn(x, vec_t),
            lambda _: models_prev[-1], (None,)
          )
        )
        return x, ts_prev, models_prev

      x, _, _ = jax.lax.fori_loop(
        order, steps + 1, multistep_loop_fn, (x, ts_prev, models_prev)
      )
      ...
      if method == 'singlestep':
        ...
        timesteps_outer = self.get_time_steps(
          skip_type=skip_type, t_T=t_T, t_0=t_0, N=K
        )
      ...
      def singlestep_loop_fn(idx, val, order):
        ...
        timesteps_inner = self.get_time_steps(
          skip_type=skip_type, t_T=t_T_inner, t_0=t_0_inner, N=order
        )
        lambda_inner = self.noise_schedule.marginal_lambda(timesteps_inner)
        # vec_s, vec_t = t_T_inner.tile(x.shape[0]), t_0_inner.tile(x.shape[0])
        # Use the following implementation as we might have time dimension
        vec_s = jnp.ones(x.shape[:-1]) * t_T_inner
        vec_t = jnp.ones(x.shape[:-1]) * t_0_inner
      ...
      for (order, nums) in order_list:
        # i, x = jax.lax.fori_loop(
        #   0, nums, lambda idx, val: singlestep_loop_fn(idx, val, order),
        #   (i, x)
        # )
        for _ in range(0, nums):
          i, x = singlestep_loop_fn(_, (i, x), order)
...
def interpolate_fn(x, xp, yp):
  ...
  end_idx = jnp.where(
    jax.lax.eq(start_idx, cand_start_idx), start_idx + 2, start_idx + 1
  )
  start_x = jnp.take_along_axis(
    sorted_all_x, jnp.expand_dims(start_idx, 2), axis=2
  ).squeeze(2)
  end_x = jnp.take_along_axis(
    sorted_all_x, jnp.expand_dims(end_idx, 2), axis=2
  ).squeeze(2)
