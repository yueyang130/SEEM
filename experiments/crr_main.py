...
def main(argv):
  ...
  crr = CRR(FLAGS.cql, policy, qf)
  sampler_policy = SamplerPolicy(crr.policy, crr.train_params['policy'])
  ...
  for epoch in range(FLAGS.n_epochs):
    ...
    with Timer() as train_timer:
      for _ in tqdm.tqdm(range(FLAGS.n_train_step_per_epoch)):
        ...
        metrics.update(
          prefix_metrics(crr.train(batch), 'crr')
        )

    with Timer() as eval_timer:
      if epoch == 0 or (epoch + 1) % FLAGS.eval_period == 0:
        trajs = eval_sampler.sample(
          sampler_policy.update_params(crr.train_params['policy']),
          FLAGS.eval_n_trajs,
          deterministic=True
        )
        ...
        if FLAGS.save_model:
          save_data = {'crr': crr, 'variant': variant, 'epoch': epoch}
