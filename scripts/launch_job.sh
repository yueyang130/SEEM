...
then
  ...
  # for level in random
  do
    PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD} --algo=${ALGO} --dataset=${TASK} --seed=${i} --env=${env}-${level}-v2"
    sleep 1
  done
  done
  ...
  do
    PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD} --algo=${ALGO} --dataset=${TASK} --seed=${i} --env=${env} --dataset rl_unplugged"
    sleep 1
  done
  ...
  do
    PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD} --algo=${ALGO} --seed=${i}  --env=antmaze-${level} --eval_n_trajs=100 --algo_cfg.bc_weight_misa=0.5 --algo_cfg.lagrange=True --algo_cfg.add_positive=True --use_layer_norm=True --algo_cfg.unbiased_grad=True --eval_period=50 --algo_cfg.use_automatic_entropy_tuning=True --algo_cfg.qf_lr=0.0001 --algo_cfg.target_action_gap=3.0 --obs_norm=False --n_epochs=2000"
    sleep 1
  done
elif [ "$TASK" = "kitchen" ]; then
  for level in complete-v0 partial-v0 mixed-v0
  do
    PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD} --algo=${ALGO} --seed=${i} --env=kitchen-${level} --n_epochs 1000 --algo_cfg.bc_weight_misa=3.0 --use_layer_norm=True --algo_cfg.detach_pi=True --algo_cfg.use_automatic_entropy_tuning=True --algo_cfg.unbiased_grad=True --obs_norm=False --algo_cfg.qf_lr=0.0001"
    sleep 1
  done
