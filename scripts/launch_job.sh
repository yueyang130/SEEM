...
then
  ...
  # for level in random
  do
    PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD} --seed=${i} --env=${env}-${level}-v2"
    sleep 1
  done
  done
  ...
  do
    PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD}  --seed=${i} --env=${env} --dataset rl_unplugged"
    sleep 1
  done
  ...
  do
    ...
    do
      PRIORITY=${PRIORITY} NS=${NS} make run cmd="${BASE_CMD}  --seed=${i} --env=${scenario}-${tp}-v0 --n_epochs 1000 --algo_cfg.use_automatic_entropy_tuning=False --algo_cfg.target_action_gap=3.0 --algo_cfg.bc_weight_misa=5.0 --algo_cfg.detach_pi=False --obs_norm=False --use_layer_norm=True"
      sleep 1
    done
  done
else
